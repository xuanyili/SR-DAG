...
def main():
    ...
    output_dir = 'output/{}'.format(config.output_path)
    ...
    with tf.Session(config=sess_config) as sess:
        ...
        for i in (range(1, config.nb_epoch + 1)):
            ...
            # Get feed dict
            feed = {actor.input_: input_batch, actor.reward_: -reward_feed[:,0], actor.cyc_: reward_feed[:,2], actor.graphs_:graphs_feed}

            summary, base_op, score_test, probs, graph_batch, \
                reward_batch, reward_avg_baseline, train_step1, train_step2, encoderoutput = sess.run([actor.merged, actor.base_op,
                actor.test_scores, actor.log_softmax, actor.graph_batch, actor.reward_batch, actor.avg_baseline, actor.train_step1,
                actor.train_step2, actor.encoder_output], feed_dict=feed)
            # _logger.info('[iter {}] input_batch:{},encoderoutput: {}'.format(i, input_batch,
            #                  encoderoutput))
            ...
            if (i+1) % lambda_iter_num == 0:
                ...
                np.save('{}/pred_G.npy'.format(config.graph_dir), np.array(graph_batch).T)

...
def read_EXPrep(args):
    '''load results from BN repository'''

    graph = np.load(args.data_dir+'/DAG.npy')
    data = np.load(args.data_dir+'/data.npy')
    (dim1, dim2) = data.shape
    data = np.reshape(data, (dim1, dim2, 1))
    return graph, data # in dictionary
...
def load_data_discrete(args, batch_size=1000, suffix='', debug = False):
    ...
    elif args.data_type == 'experiment':
        G, X = read_EXPrep(args)
